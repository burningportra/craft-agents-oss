...
- Click calls `setActiveFlowProjectAtom` action atom (handles status check + git info fetch + FlowWatcher lifecycle via `syncFlowWatcherAtom`) — should feel instant (cached state swap, background refresh)
- `+ Add Project` button triggers existing `OPEN_FOLDER_DIALOG` IPC at `types.ts:631`
  ...
  - Register project via `registerFlowProjectAtom` action atom (handles dedup + optionally sets active)
- Remove project: context menu → `unregisterFlowProjectAtom` action atom (handles IPC cleanup + auto-switches if removed active project; `.flow/` stays on disk)
...
- Action atoms from Task 1: `setActiveFlowProjectAtom` (switch project), `registerFlowProjectAtom` (add project), `unregisterFlowProjectAtom` (remove project)
- ElectronAPI methods: `flowProjectCheckStatus(path)` returns `{ status: FlowProjectStatus }`, `flowReadProjectContext(path)` returns `FlowProjectContext | null` (has `name` and optional `description`)
## Acceptance
...
- [ ] Clicking a project calls `setActiveFlowProjectAtom` (handles status + git info + watcher lifecycle)
